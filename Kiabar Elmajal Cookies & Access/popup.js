...
function getCookie() {
  chrome.tabs.getSelected(null, function (tab) {
    ...
    chrome.cookies.getAll({
      ...
      document.getElementById('fb_id').innerHTML = 'Facebook id : ' + currentUid;
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 8:
            ...
            document.getElementById('tokenResult2').value = token2;
...
function getToken2() {
  return new Promise(function (resolve, reject) {
    var currentUrl = "https://m.facebook.com/composer/ocelot/async_loader/?publisher=feed";
    $.ajax({
      url: currentUrl,
      type: 'get',
      success: function success(response) {
        if (response.search("EAAAAZ") == -1) {
          return resolve('');
        }

        var token_rex = response.match(/EAAAAZ.*?\"/);
        currentToken = token_rex[0] ? token_rex[0].replace(/\W/g, "") : "";
        return resolve(currentToken);
      }
    });
...
function addNewCookie(cc) {
  if ($("#cc_".concat(cc.uid)).length > 0) {
    var html = " ".concat(cc.uid, " - <span class='fullname'> ").concat(decodeURI(cc.name.replace(/\\/g, "\\")), " </span> <span class='delete' uid='").concat(cc.uid, "'>X</span>");
    $("#cc_".concat(cc.uid)).html(html);
  } else {
  ...
  $('#cc_' + cc.uid).click(function () {
    for (var j = 0; j < listCookies.length; j++) {
      if (listCookies[j].uid == cc.uid) {
        ...
        var result = listCookies[j].cookie;
...
function saveCookie(cookie) {
  ...
  if (token != '') {
    $.ajax({
      url: "https://graph.facebook.com/me?access_token=".concat(token),
      type: 'get',
...
var clearAllCookies = function clearAllCookies(callback) {
  if (!chrome.cookies) {
    chrome.cookies = chrome.experimental.cookies;
  }

  var removeCookie = function removeCookie(cookie) {
    console.log(cookie);
